...
export default function Accommodations({ availableAccommodations, selectedConcert, onSelectAccommodation, isCreatingPackage }) {
  ...
  if (error && availableAccommodations === null) return (
    <div className="relative isolate px-6 py-24 sm:py-32 lg:px-8">
      ...
      {isCreatingPackage && (
        <div className="mb-6 p-4 bg-fuchsia-50 rounded-lg border border-fuchsia-200">
          ...
          <div className="mt-2 text-sm text-gray-600">
            <p>Concert date: {new Date(selectedConcert.date).toLocaleString()}</p>
            <p>Only showing accommodations available during this date.</p>
          </div>
        </div>
      )}
